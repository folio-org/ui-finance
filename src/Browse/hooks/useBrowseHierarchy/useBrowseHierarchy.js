...
export const useBrowseHierarchy = (fiscalYearId, options = {}) => {
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      // 1. Fetch finance data for this fiscal year
      const financeDataResponse = await ky.get(FINANCE_DATA_API, {
        searchParams: {
      ...
      if (financeData.length === 0) {
        // Fallback: try to fetch all ledgers and filter
        const ledgersResponse = await ky.get(LEDGERS_API, {
          searchParams: {
        ...
        // Fetch budgets for this fiscal year to get fund associations
        ...
        // Fetch all groups
        const groupsResponse = await ky.get(GROUPS_API, {
          searchParams: {
            query: 'cql.allRecords=1',
            limit: LIMIT_MAX,
          },
      ...
      const hierarchy = buildHierarchyFromFinanceData(financeData);
      const counts = calculateCounts(hierarchy);
 ...
 */
const buildHierarchyFromFinanceData = (financeData) => {
  // Group by ledger
  ...
  financeData.forEach(item => {
    ...
    if (!ledgerMap.has(ledgerId)) {
      ledgerMap.set(ledgerId, {
        id: ledgerId,
        type: 'ledger',
        name: item.ledgerName || ledgerCode || 'Unknown Ledger',
        code: ledgerCode,
        status: item.ledgerStatus || 'Active',
        groupsMap: new Map(),
      });
    }
    ...
    if (!ledger.groupsMap.has(groupId)) {
      ledger.groupsMap.set(groupId, {
        id: groupId,
        type: 'group',
        name: groupId === UNGROUPED_ID ? UNGROUPED_NAME : (item.groupName || groupCode || 'Unknown Group'),
        code: groupCode,
...
const calculateCounts = (hierarchy) => {
  let ledgersCount = hierarchy.length;
  let groupsCount = 0;
  let fundsCount = 0;
  let budgetsCount = 0;
